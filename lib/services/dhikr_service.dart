...
class DhikrService {
  ...
  static final List<Dhikr> _dhikrs = [
    Dhikr(id: 'subhanallah', arabic: 'سُبْحَانَ اللَّهِ', transliteration: 'Subhanallah', meaning: 'Glory be to Allah', target: 33, category: 'tasbih'),
    Dhikr(id: 'alhamdulillah', arabic: 'الْحَمْدُ لِلَّهِ', transliteration: 'Alhamdulillah', meaning: 'Praise be to Allah', target: 33, category: 'tasbih'),
    Dhikr(id: 'allahu_akbar', arabic: 'اللَّهُ أَكْبَرُ', transliteration: 'Allahu Akbar', meaning: 'Allah is the Greatest', target: 34, category: 'tasbih'),
    Dhikr(id: 'la_ilaha_illallah', arabic: 'لَا إِلَٰهَ إِلَّا اللَّهُ', transliteration: 'La ilaha illallah', meaning: 'There is no god but Allah', target: 100, category: 'tahlil'),
    Dhikr(id: 'astaghfirullah', arabic: 'أَسْتَغْفِرُ اللَّهَ', transliteration: 'Astaghfirullah', meaning: 'I seek forgiveness from Allah', target: 100, category: 'istighfar'),
    Dhikr(id: 'dorud_sharif', arabic: 'صَلَّى اللَّهُ عَلَيْهِ وَسَلَّمَ', transliteration: 'Dorud Sharif', meaning: 'Peace and blessings be upon him', target: 100, category: 'dorud'),
  ];

  static final List<AllahName> _allahNames = [
    for (int i = 1; i <= 99; i++)
      AllahName(number: i, arabic: 'اسم $i', transliteration: 'Name $i', meaning: 'Meaning $i'),
  ];
  ...
  static Future<Map<String, DhikrCount>> getDhikrCounts() async {
    ...
    if (countsJson != null) {
      ...
      return data.map((key, value) => MapEntry(key, DhikrCount.fromJson(value)));
    }
    return {};
  }
  ...
  static Future<int> getTodayDhikrCount() async {
    ...
    for (var count in counts.values) {
      if (count.lastUpdated.year == today.year && count.lastUpdated.month == today.month && count.lastUpdated.day == today.day) {
        total += count.count;
      }
    }
    return total;
